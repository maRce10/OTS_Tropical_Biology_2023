---
title: <font size="7"><b>Linear regression models</b></font>
...
plot_model <- function(...) sjPlot::plot_model(colors = viridis(10, alpha = 0.5)[7], ...)
...
- [Use simulated data to understand linear models](#how-to-use-simulated-data-to-understand-the-behavior-of-statistical-tools)
...
<font size = 6>Simple linear regression</font>
...
<font size = 6>$\hat{Y} \sim \beta_{o} + \beta_{1} * x_{1}$</font>
...
<font size = 6>$\qquad \mathcal{H}_0: \hat{Y} = \beta_0 + 0 * x_{1}$</font>

which is equivalent to this:

<font size = 6>$\qquad \mathcal{H}_0: \hat{Y} = \beta_0$</font>
...
<font size = 6>$\hat{Y} \sim 1.079 + 2.926 * x_{1}$</font>
...
<font size = 6>$\hat{Y} \sim \beta_{o}$</font>
...
- Change the `mean` argument in the `rnorm()` function call ([line 111](#cb53-8)) to a value other than 0 and look at how things change in the coefficients table 
...
<font size = 6>$\hat{Y} \sim \beta_{o} + \beta_{1} * x_{1} + \cdots + \beta_{n} * x_{n}$</font>
...
<font size = 6>$\hat{Y} \sim \beta_{o} + \beta_{1} * x_{1} + \beta_{2} * x_{2}$</font>
...
ggplot(xy_data_cat, aes(x = x1, y = y)) + 
  geom_boxplot() + geom_point(data = aggregate(y ~ x1, xy_data_cat, mean), col = "#31688EFF", size = 2)
...
ggplot(xy_data_cat, aes(x = x1, y = y)) + 
  geom_boxplot() +
geom_hline(yintercept = xy_mod_cat$coefficients[1], col = "blue") + geom_point(data = aggregate(y ~ x1, xy_data_cat, mean), col = "#31688EFF", size = 2)
...
ggplot(xy_data_cat, aes(x = x1, y = y)) + 
  ...
  geom_point(data = aggregate(y ~ x1, xy_data_cat, mean), col = "#31688EFF", size = 2)
...
- Unbalanced data when having categories (i.e. some categories have way more observations than others) can be problematic for statistical inference. Modify the code [above](#cb100-1) to simulate a highly unbalanced data set and check the precision of the model (tip: )
...
<font size = 6>$\hat{Y} \sim \beta_{o} + \beta_{1} * x_{1} + \beta_{2} * x_{2} + \beta_{3} * x_{1} * x_{2}$</font>
...
- How is the inference affected by the presence of collinear predictors? Make the diagnostic plots for this model (`plot_model(model_name, type = "diag")`).
